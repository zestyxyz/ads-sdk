...
const getImageSrc = async (banner) => {
  return await (
    await (
      await (
        await (
          await (await banner.getProperty('components')).getProperty('material')
        ).getProperty('data')
      ).getProperty('src')
...
describe('Initial load', () => {
  ...
  test('The wide banner is present', async () => {
    const banner = await page.evaluate(() =>
      document.querySelector('#banner2').getAttribute('zesty-banner')
    );
    expect(banner).not.toBeFalsy();
  });
  ...
  test('The square banner is present', async () => {
    const banner = await page.evaluate(() =>
      document.querySelector('#banner3').getAttribute('zesty-banner')
    );
    expect(banner).not.toBeFalsy();
  });
});
